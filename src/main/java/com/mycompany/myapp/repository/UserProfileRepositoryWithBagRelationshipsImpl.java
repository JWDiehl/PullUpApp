...
public class UserProfileRepositoryWithBagRelationshipsImpl implements UserProfileRepositoryWithBagRelationships {
    ...
    public Optional<UserProfile> fetchBagRelationships(Optional<UserProfile> userProfile) {
        return userProfile.map(this::fetchSavedCourts);
    }
    ...
    public List<UserProfile> fetchBagRelationships(List<UserProfile> userProfiles) {
        return Optional.of(userProfiles).map(this::fetchSavedCourts).orElse(Collections.emptyList());
    }

    UserProfile fetchSavedCourts(UserProfile result) {
        return entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.savedCourts where userProfile.id = :id",
                UserProfile.class
            )
    ...
    List<UserProfile> fetchSavedCourts(List<UserProfile> userProfiles) {
        ...
        List<UserProfile> result = entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.savedCourts where userProfile in :userProfiles",
                UserProfile.class
            )
