...
import { sampleWithRequiredData } from '../court-type.test-samples';
...
describe('CourtType Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.courtTypes?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to courtTypeService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getCourtTypeIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getCourtTypeIdentifier).toHaveBeenCalledWith(entity);
